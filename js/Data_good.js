...
let dataGood = (function() {
  ...
  function getGoods() {
    let goods = JSON.parse(localStorage.getItem("allGoods"));
    return goods;
  }
  ...
  function addDefaultGoods() {
    let goods = getGoods();
    if (!goods) {
      localStorage.setItem("allGoods", JSON.stringify(goodsData));
    }
  }
  ...
  function addGood(good) {
    let goods = getGoods();
    goods.unshift(good);
    localStorage.setItem("allGoods", JSON.stringify(goods));
  }

  function tryDeleteGoodByTitle(title) {
    let goods = getGoods();
    let goodIndex = goods.findIndex(good => good.title === title);
  ...
  return {
    getGoods: getGoods,
    addDefaultGoods: addDefaultGoods,
    addGood: addGood,
    tryDeleteGoodByTitle: tryDeleteGoodByTitle
  };
})();
